...
echo "🎨 Applying Material-ish UI + mobile responsiveness (fixed script)..."

# 0) Ensure dirs exist
mkdir -p packages/ui/styles
mkdir -p packages/ui/src/components
mkdir -p apps/web-superadmin/components/shell
mkdir -p "apps/web-superadmin/app/(sa)/churches"

# 1) Export CSS files from @noxera/ui (so Next can import them)
node - <<'NODE'
const fs = require("fs");
const p = "packages/ui/package.json";
const j = JSON.parse(fs.readFileSync(p, "utf8"));

j.sideEffects = Array.from(new Set([...(j.sideEffects || []), "./styles/*.css"]));

const exp = j.exports && typeof j.exports === "object" ? j.exports : {};
// preserve existing exports, add styles subpath
exp["."] = exp["."] || "./src/index.ts";
exp["./styles/*"] = "./styles/*";
j.exports = exp;

fs.writeFileSync(p, JSON.stringify(j, null, 2) + "\n");
console.log("✅ packages/ui/package.json exports + sideEffects updated");
NODE

# 2) Design tokens + base glow
...
cat > packages/ui/styles/index.css <<'CSS'
@import "./tokens.css";
@import "./base.css";
CSS

# 3) Improve Card/Button/Badge + Progress
...
# Ensure @noxera/ui exports Progress + lucide icons
if ! grep -q 'export \* from "\.\/components\/progress"' packages/ui/src/index.ts 2>/dev/null; then
  echo '' >> packages/ui/src/index.ts
  echo 'export * from "./components/progress";' >> packages/ui/src/index.ts
fi
if ! grep -q 'export \* from "lucide-react"' packages/ui/src/index.ts 2>/dev/null; then
  echo '' >> packages/ui/src/index.ts
  echo 'export * from "lucide-react";' >> packages/ui/src/index.ts
fi

# 4) Make apps import the UI styles (add once)
for app in apps/web-superadmin apps/web-church apps/web-public; do
  CSSFILE="$app/app/globals.css"
  if [ -f "$CSSFILE" ]; then
    if ! grep -q '@noxera/ui/styles/index.css' "$CSSFILE"; then
      # add near top
      perl -0777 -pi -e 's/^/@import "@noxera\/ui\/styles\/index.css";\n/s' "$CSSFILE"
      echo "✅ Injected UI styles import into $CSSFILE"
    fi
  fi
done

# 5) Mobile-responsive SuperAdminShell (drawer on mobile)
...
# 6) Church Directory page (KPI cards + progress + mobile cards)
    ...
    <div className="space-y-5">
      ...
      <div className="hidden md:block overflow-hidden rounded-2xl border border-border/70 bg-card/70 backdrop-blur">
        ...
        {filtered.length === 0 ? (
          ...
          filtered.map((t) => (
            <div key={t.id} className="grid grid-cols-[2fr_1fr_1fr_1fr_1fr] items-center border-t border-border/70 px-4 py-4 transition hover:bg-accent/40">
              <div>
...
echo "✅ Material UI upgrade (fixed) applied."
